...
Див. `DEPLOY.md`. Коротко: пуш у `main` → Render автобілд з `Dockerfile` → **два сервіси** з одного образу: `binotel-poller` (Cron Job, `*/15 * * * *`, `JOB_TYPE=poll`) і `binotel-bot` (Background Worker, `node src/bot/index.js`). Регіон Frankfurt. ~$1/міс cron + ~$7/міс worker + OpenAI.
