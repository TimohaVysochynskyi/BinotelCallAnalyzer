...
2. **📊 Статистика менеджера** (`stats.js`, admin) — менеджер → період → `deliverManagerReport` (audio:true): числовий header (**всього / продажних / інформаційних**; конверсія й найслабший етап — по продажних) + findings (≥3 реальних релевантних цитати) + аудіо-кліпи під негатив + готові фрази. Кнопки нотаток. Двошаровий пайплайн з кешем: per-call MAP (`core/analyzeCall.js` → `calls.behaviors`+`call_purpose`; **непродажні дзвінки не дають findings**, цитата — лише з реплік менеджера) + REDUCE (`analyze.js: reduceFindings`) + чиста код-верифікація (`assembleFindings`: дедуп, ≥3, суворий матч по менеджеру) + **LLM-прохід релевантності** (`verifyFindingsRelevance`). Звіт рендериться **завжди** (нема findings → чіткий рядок, не «недостатньо матеріалу»).
