...
1. **🔄 Звіт зараз / авто-звіти** — планувальник (`report.js`) у слоти з `/settings` → «🕒 Час звітів» (`app_state.report_times`, дефолт 13:00/19:30 Kyiv, поки не задано) — in-process `setInterval` у воркері бота, не окремий cron. Період — з моменту попереднього авто-звіту (стан у `app_state.last_report_slot`/`last_report_until`). На кожного менеджера — OpenAI-резюме (`analyze.js`) → **PDF** (`pdfReport.js`, `pdfkit`, одна сторінка на менеджера). PDF будується раз і розсилається всім отримувачам зі списку `/settings` («Щоденні звіти», `app_state.report_recipients`) — не env. «Звіт зараз» — ручний, за сьогодні, той самий PDF-формат; шле **тому, хто натиснув** (не в список), стан розкладу не чіпає.
2. **📊 Статистика менеджера** (`stats.js`) — обрати менеджера → період (день / тиждень з пн / місяць / квартал) → **короткий числовий блок зверху** (к-сть дзвінків, успішність %, середній бал, найчастіший слабкий етап) + **розгорнутий AI-аналіз за період** нижче (`analyzeManager`, той самий промпт із `/prompt`, що й у звітах). Те саме в «Мій звіт». Кнопки: додати нотатку (`manager_notes`), переглянути нотатки. Слабкі етапи задаються в `core/classifyCall.js` (`WEAKEST_STAGES`, на інжесті). ⚠️ Аналіз поки генерується щоразу (OpenAI ~20-40с, без кешу) — кеш/швидкодія заплановані окремо.
3. **🗂 Архів розмов** (`archive.js`) — менеджер → період → список дзвінків (пагінація) → конкретний дзвінок: **розмова у форматі діалогу** (`dialogue.js: formatDialogue` — AI розбиває суцільний моно-транскрипт на репліки «Менеджер:» / «Клієнт:», не переписуючи слова; фейл → сирий текст) + кнопка «🎧 Прослухати запис» (тягне свіжий record-URL з Binotel і шле аудіо). У списку менеджерів біля імені — період активності `dd.mm.yy-dd.mm.yy` (`operatorListKeyboard(..., {showDates:true})`). ⚠️ Діалог генерується щоразу без кешу (окрема задача оптимізації).
...
8. **📊 Мій звіт** (`me:*` у `stats.js`, `/myreport`, роль manager) — стата ЛИШЕ по собі (за `bot_users.operator_name`) + свій телефон; кнопка «☎️ Оновити мій номер» (request_contact).
...
stats.js      — фіча 2 (числовий блок + AI-аналіз за період через analyzeManager + нотатки); також «Мій звіт»
archive.js    — фіча 3 (архів + аудіо + діалог через dialogue.js)
dialogue.js   — фіча 7 (Задача): AI-розділення мовців, суцільний транскрипт → діалог «Менеджер:»/«Клієнт:»
